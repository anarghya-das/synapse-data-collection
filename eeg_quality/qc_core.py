...
VENDORED into the data-collection repo from ``synapse-data/synapse_qc/qc_core.py``
so the live recording gate (``recording_checks.py``) scores signal quality with
the EXACT same algorithm the analysis pipeline uses to accept/reject a subject.
If the QC changes in synapse-data, re-copy this file. Do not edit it here.
